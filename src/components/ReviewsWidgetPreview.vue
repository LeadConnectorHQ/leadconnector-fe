...
const props = withDefaults(defineProps<Props>(), {
  ...
  locationId: '',
  deviceView: 'desktop',
  scriptUrl: config.LC_REPUTATION_WIDGET_SCRIPT_URL,
  baseUrl: config.LC_REPUTATION_WIDGET_BASE_URL,
  widgetType: '',
  widgetPosition: ''
});
...
const containerClasses = computed(() => [
  'w-full',
  props.deviceView === 'mobile' && 'max-w-[400px] mx-auto'
].filter(Boolean));
...
const isBottomPosition = (position: string): boolean => {
  return position === 'bottom' || position === 'bottom-right' || position === 'bottom-left';
};
...
const ensureBottomPositionOnHeightChange = () => {
  if (props.widgetType === 'flash' && isBottomPosition(props.widgetPosition) && iframeRef.value) {
...
const handleIframeMessage = (event: MessageEvent): void => {
  if (event.data && Array.isArray(event.data)) {
    ...
    if ((eventName === 'lc.setHeight' || eventName === 'lc.setFlashHeight') && data?.id === 'lc_reviews_widget' && data?.height) {
...
watch(iframeHeight, (newHeight) => {
  if (props.widgetType === 'flash' && isBottomPosition(props.widgetPosition) && newHeight > 0) {
    ensureBottomPositionOnHeightChange();
  }
});

watch(() => props.widgetId, (newWidgetId) => {
  if (newWidgetId) {
    iframeLoaded.value = false;
  }
});

watch(() => props.deviceView, () => {
  iframeLoaded.value = false;
  iframeKey.value += 1;
});
